...
fn main() {
    ...
    // Input de Firerate
    let mut frame = Frame::default().with_pos(10, y).with_size(150, 25).with_label("Firerate (SPM):");
    frame.set_align(Align::Left | Align::Inside);
    let rate_input = Input::default().with_pos(170, y).with_size(100, 25);
    y += 30;

    // Botão de Calcular
    let mut calc_btn = Button::default().with_pos(10, y).with_size(260, 35).with_label("Calculate");

    // Display de Resultado (Tabela)
    let mut out_disp = TextDisplay::default().with_pos(290, 10).with_size(520, 330);
    ...
    calc_btn.set_callback(move |_| {
        let mut process = || -> Result<String, String> {
